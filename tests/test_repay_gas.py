...
def test_repay_gas(
    ...
    with test_utils.GasWatcher(["addQuoteToken", "addCollateral", "repay", "borrow"]):
        ...
        mkr_dai_pool.addCollateral(100 * 10**27, {"from": borrowers[0]})

        # borrow 10_000 DAI from single bucket (LUP)
        mkr_dai_pool.borrow(10_000 * 10**45, 1 * 10**18, {"from": borrowers[0]})
        tx_repay_to_one_bucket = mkr_dai_pool.repay(
            10_001 * 10**45, {"from": borrowers[0]}
        )

        # borrow 101_000 DAI from 11 buckets
        mkr_dai_pool.borrow(101_000 * 10**45, 1 * 10**18, {"from": borrowers[0]})
        tx_repay_to_11_buckets = mkr_dai_pool.repay(
            101_001 * 10**45, {"from": borrowers[0]}
        )
