...
def test_update_interest_rate_gas(
    ...
    with test_utils.GasWatcher(
        ...
        mkr_dai_pool.addQuoteToken(
            lender, 10_000 * 10**45, bucket_math.indexToPrice(1663), {"from": lender}
        )
        mkr_dai_pool.addQuoteToken(
            lender, 10_000 * 10**45, bucket_math.indexToPrice(1637), {"from": lender}
        )
        mkr_dai_pool.addQuoteToken(
            lender, 10_000 * 10**45, bucket_math.indexToPrice(1569), {"from": lender}
        )

        # borrower deposits 100 MKR collateral and draws debt
        mkr_dai_pool.addCollateral(100 * 10**27, {"from": borrower1})
        mkr_dai_pool.borrow(25_000 * 10**45, 2500 * 10**18, {"from": borrower1})
