 ...
 */ 
contract UnboundedLenderHandler is Test {
    ...
    function addQuoteToken(uint256 amount, uint256 bucketIndex) internal {
        numCalls++;
        numberOfCalls['UBLenderHandler.addQuoteToken']++;
    ...
    function removeQuoteToken(uint256 amount, uint256 bucketIndex) internal {
        ...
        numCalls++;
        numberOfCalls['UBLenderHandler.removeQuoteToken']++;
 ...
 *  @dev randomly selects a lender contract to make a txn
 */ 
contract BoundedLenderHandler is UnboundedLenderHandler {

    constructor(address pool, address quote, address collateral, address poolInfo, uint256 numOfActors) UnboundedLenderHandler(pool, quote, collateral, poolInfo, numOfActors) {} 

    function addQuoteToken(uint256 actorIndex, uint256 amount, uint256 bucketIndex) public useRandomActor(actorIndex) useRandomBucketLender(bucketIndex) {
        numCalls++;
        numberOfCalls['BLenderHandler.addQuoteToken']++;
        ...
        // Action
        super.addQuoteToken(amount, _bucketLender);

        // Post condition
        // run asserts here
    }

    function removeQuoteToken(uint256 actorIndex, uint256 amount, uint256 bucketIndex) public useRandomActor(actorIndex) useRandomBucketLender(bucketIndex) {

        numCalls++;
        numberOfCalls['BLenderHandler.removeQuoteToken']++;
        ...
        // Action
        super.removeQuoteToken(amount, _bucketLender);
