...
contract BasicInvariants is TestBase {
    ...
    ****************************************************************************************************************************************/
    ...
    function invariant_quoteTokenBalance_QT1() public {
        ...
        (uint256 totalBondEscrowed, uint256 totalClaimableBond, uint256 unClaimed, , ) = _pool.reservesInfo();

        assertGe(poolBalance + poolDebt, totalBondEscrowed + totalClaimableBond + _pool.depositSize() + unClaimed, "Incorrect pool quote token");
    }
