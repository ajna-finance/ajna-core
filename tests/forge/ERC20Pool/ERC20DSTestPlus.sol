...
abstract contract ERC20DSTestPlus is DSTestPlus {
    ...
    function redeemLendersLp(
        ...
        for(uint j = 0; j < indexes.length(); j++ ){
            ...
            if(lenderLpBalance != 0 && bucketCollateral != 0) {
                (, lpRedeemed) = ERC20Pool(address(_pool)).redeemLPforCollateral(type(uint256).max, bucketIndex);
                lenderLpBalance -= lpRedeemed;
            }
    ...
    function _reedemLPforCollateral(
        address from,
        uint256 lpAmount,
        uint256 index,
        uint256 collateralRemove,
        uint256 lpRedeem
    ) internal {
        changePrank(from);
        vm.expectEmit(true, true, true, true);
        emit RemoveCollateral(from, index, collateralRemove);
        vm.expectEmit(true, true, true, true);
        emit Transfer(address(_pool), from, collateralRemove);
        (uint256 collateralRemoved, uint256 lpRedeemed) = ERC20Pool(address(_pool)).redeemLPforCollateral(
            lpAmount, 
            index
        );
        assertEq(collateralRemoved, collateralRemove);
    ...
    function _assertRemoveAllCollateralNoClaimRevert(
        ...
        ERC20Pool(address(_pool)).redeemLPforCollateral(type(uint256).max, index);
    }
