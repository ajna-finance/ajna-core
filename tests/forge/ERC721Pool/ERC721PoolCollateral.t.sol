...
contract ERC721PoolCollateralTest is ERC721HelperContract {
    ...
    function testMergeERC721OrRemoveCollateral() external tearDown {
        ...
        _assertBucket({
            ...
            lpBalance:    20.0 * 1e18,
            collateral:   0.0 * 1e18,
            deposit:      20.004183919163565300 * 1e18,
            exchangeRate: 1.000209195958178265 * 1e18
        });
        ...
        _assertAuction(
            AuctionParams({
                ...
                totalBondEscrowed: 5.907892673985352325 * 1e18,
                auctionPrice:      0.000004621809202112 * 1e18,
                debtInAuction:     439.677389340513210329 * 1e18,
                thresholdPrice:    385.776675964868418456 * 1e18,
                neutralPrice:      310.164365384230997074 * 1e18
            })
        );
        ...
        _assertPool(
            PoolParams({
                htp:                  0,
                lup:                  99836282890,
                poolSize:             374.170934385060477538 * 1e18,
                pledgedCollateral:    1.139719990175231268 * 1e18,
        ...
        _assertBorrower({
            borrower:                  _borrower,
            borrowerDebt:              439.677389340513210329 * 1e18,
            borrowerCollateral:        1.139719990175231268 * 1e18,
            borrowert0Np:              78.825721153846153882 * 1e18,
            borrowerCollateralization: 0.000000000258792947 * 1e18,
            tokenIds:                  borrowerTokenIds
        });
        ...
        _assertBorrower({
            borrower:                  _borrower,
            borrowerDebt:              439.677384764922100239 * 1e18,
            borrowerCollateral:        0.139719990175231268 * 1e18,
            borrowert0Np:              78.825721153846153882 * 1e18,
            borrowerCollateralization: 0.000000000031725817 * 1e18,
            tokenIds:                  borrowerTokenIds
        });
        ...
        _assertAuction(
            AuctionParams({
                ...
                auctionPrice:      0 * 1e18,
                debtInAuction:     439.677384764922100239 * 1e18,
                thresholdPrice:    3_148.371989379999109069 * 1e18,
                neutralPrice:      310.164365384230997074 * 1e18
            })
        );
        ...
        _settle({
            ...
            borrower:    _borrower,
            maxDepth:    10,
            settledDebt: 111.717941412250938348 * 1e18
        });
        ...
        _assertBucket({
            index:        3070,
            lpBalance:    0,                             // new LPs amount accounting collateral merged in bucket
            collateral:   0,                             // reflects collateral merged in the bucket
            deposit:      0,
            exchangeRate: 1 * 1e18
        });
        _assertLenderLpBalance({
        ...
        _assertBucket({
            index:        7388,
            lpBalance:    0,                           // LPs awarded to borrower for settled collateral
            collateral:   0,                           // settled collateral amount
            deposit:      0,
            exchangeRate: 1 * 1e18
        });
        ...
        _assertPool(
            PoolParams({
                htp:                  0,
                lup:                  MAX_PRICE,
                poolSize:             50.000004575591110080 * 1e18,
                pledgedCollateral:    0,
                encumberedCollateral: 0,
                poolDebt:             0,
                actualUtilization:    808.632216044431490433 * 1e18,
