...
abstract contract UnboundedPositionPoolHandler is UnboundedBasePositionHandler, BaseHandler {
    ...
    function _memorializePositions(
        ...
        for(uint256 i = 0; i < indexes_.length; i++) {
            ...
            LenderInfo memory lenderInfoBeforeAction = _getLenderInfo(indexes_[i], address(_actor));
            LenderInfo memory positionManagerInfoBeforeAction = _getLenderInfo(indexes_[i], address(_positionManager));

            actorLpsBefore[address(_pool)][indexes_[i]]  = lenderInfoBeforeAction.lpBalance;
            posManLpsBefore[address(_pool)][indexes_[i]] = positionManagerInfoBeforeAction.lpBalance;

            // positionManager is assigned the most recent depositTime
            bucketIndexToDepositTime[address(_pool)][indexes_[i]] = (lenderInfoBeforeAction.depositTime >= positionManagerInfoBeforeAction.depositTime) ?
                lenderInfoBeforeAction.depositTime : positionManagerInfoBeforeAction.depositTime;
        }

        try _positionManager.memorializePositions(
            address(_pool),
            tokenId_,
            indexes_
        ) {
            ...
            for ( uint256 i = 0; i < indexes_.length; i++) {
                ...
                require(
                    positionManagerInfoBeforeAction.depositTime == bucketIndexToDepositTime[address(_pool)][bucketIndex],
                ...
                require(
                    positionManagerInfoBeforeAction.lpBalance == actorLpsBefore[address(_pool)][bucketIndex] + posManLpsBefore[address(_pool)][bucketIndex],
                ...
                (
                    uint256 posLps,
                ) = _positionManager.getPositionInfo(tokenId_, bucketIndex);

                require(
                    posLps == actorLpsBefore[address(_pool)][bucketIndex],
    ...
    function _mint() internal returns (uint256 tokenIdResult) {
        ...
        try _positionManager.mint(
            ...
            require(
                _positionManager.ownerOf(tokenId) == _actor,
                "PM4: minter is not owner"
            );
            ...
            require(
                ...
                "PM4: poolKey does not match pool address"
            );
            ...
            require(
                posIndexes.length == 0,
                "PM4: positions are associated with tokenId"
            );
        } catch (bytes memory err) {
            _ensurePositionsManagerError(err);
        }
    ...
    function _redeemPositions(
        ...
        for (uint256 i = 0; i < indexes_.length; i++) {
            ...
            actorLpsBefore[address(_pool)][indexes_[i]]  = _getLenderInfo(indexes_[i], preActionOwner).lpBalance;
            posManLpsBefore[address(_pool)][indexes_[i]] = _getLenderInfo(indexes_[i], address(_positionManager)).lpBalance;
        } 

        try _positionManager.redeemPositions(
            address(_pool),
            tokenId_,
            indexes_) {
            ...
            for ( uint256 i = 0; i < indexes_.length; i++) {
                ...
                uint256 poolActorLps = _getLenderInfo(bucketIndex, preActionOwner).lpBalance;
                uint256 poolPosLps   = _getLenderInfo(bucketIndex, address(_positionManager)).lpBalance;

                // assert PositionsMan LP in pool matches the amount redeemed by actor 
                // positionMan has now == positionMan pre - actor's LP change
                require(
                    poolPosLps == posManLpsBefore[address(_pool)][bucketIndex] - (poolActorLps - actorLpsBefore[address(_pool)][bucketIndex]),
                ...
                require(
                    poolActorLps == actorLpsBefore[address(_pool)][bucketIndex] + (posManLpsBefore[address(_pool)][bucketIndex] - poolPosLps), 
                ...
                (
                    uint256 posLps,
                    uint256 posDepositTime
                ) = _positionManager.getPositionInfo(tokenId_, bucketIndex);

                require(
                    posLps == 0,
                    "PM8: tokenId has lps after redemption"
                );
                require(
                    posDepositTime == 0,
                    "PM8: tokenId has depositTime after redemption"
                );
            ...
            if (totalPositionIndexes == indexes_.length) {
                ...
                require(
                    posIndexes.length == 0,
                    "PM8: positions still exist after redemption"
                );
    ...
    function _getQuoteAtIndex(
        ...
        BucketInfo memory bucketInfo = _getBucketInfo(index);
    ...
    function _moveLiquidity(
        ...
        try _positionManager.moveLiquidity(
            address(_pool),
            tokenId_,
            fromIndex_,
            toIndex_,
            ...
            (
                uint256 fromLps,
                uint256 fromDepositTime
            ) = _positionManager.getPositionInfo(tokenId_, fromIndex_);

            require(
                fromLps == 0,
                "PM6: from bucket still has LPs after move"
            );
            require(
                fromDepositTime == 0,
                "PM6: from bucket still has deposit time after move"
            );

            // assert that toIndex LP is increased and deposit time matches positionManagers depositTime pre action
            uint256 postActionDepositTime= _getLenderInfo(toIndex_, address(_positionManager)).depositTime;
            (
                uint256 toLps,
                uint256 toDepositTime
            ) = _positionManager.getPositionInfo(tokenId_, toIndex_);

            require(
                toLps >= preActionToLps,
                "PM6: to bucket lps have not increased"
            ); // difficult to estimate LPS, assert that it is greater than
            require(
                toDepositTime == postActionDepositTime,
                "PM6: to bucket deposit time does not match positionManager"
            ); 
    ...
    function _burn(
        ...
        try _positionManager.burn(
            ...
            require(
                _positionManager.ownerOf(tokenId_) == address(0),
                "PM5: ownership is not zero address"
            );
            ...
            require(
                ...
                "PM5: poolKey has not been reset on burn"
            );
            ...
            require(
                posIndexes.length == 0,
                "PM5: positions still exist after burn"
            );
        } catch (bytes memory err) {
            _ensurePositionsManagerError(err);
        }
    ...
    function _transferPosition(
        ...
        try _positionManager.transferFrom(
            _actor,
            receiver_,
            tokenId_
        ) {
            ...
            require(
                _positionManager.ownerOf(tokenId_) == receiver_,
                "new NFT owner should be receiver"
            );
        } catch (bytes memory err) {
            _ensurePositionsManagerError(err);
        }
