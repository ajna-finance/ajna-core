...
abstract contract UnboundedRewardsPoolHandler is UnboundedPositionPoolHandler {
    ...
    function _unstake(
        ...
        for (uint256 epoch = preActionLastClaimedEpoch; epoch <= _pool.currentBurnEpoch(); epoch++) {
            ...
            if (_rewardsManager.isEpochClaimed(tokenId_, epoch)) {
                rewardsEarnedInEpochPreAction[epoch] = _rewardsManager.rewardsClaimed(address(_pool), epoch);
            }
            
            // total the rewards earned pre action
            totalRewardsEarnedPreAction  += _rewardsManager.rewardsClaimed(address(_pool), epoch) + _rewardsManager.updateRewardsClaimed(address(_pool), epoch);
        } 
        ...
        try _rewardsManager.unstake(tokenId_) {
            ...
            for (uint256 epoch = preActionLastClaimedEpoch; epoch <= _pool.currentBurnEpoch(); epoch++) {
                ...
                if (rewardsEarnedInEpochPreAction[epoch] > 0) {
                    require(rewardsEarnedInEpochPreAction[epoch] == _rewardsManager.rewardsClaimed(address(_pool), epoch), 
                    "RW10: staker has claimed rewards from the same epoch twice"); 
                }

                // total rewards earned across all actors in epoch post action
                totalRewardsEarnedPostAction += _rewardsManager.rewardsClaimed(address(_pool), epoch) + _rewardsManager.updateRewardsClaimed(address(_pool), epoch);

                // reset staking and updating rewards earned in epoch
                rewardsClaimedPerEpoch[address(_pool)][epoch]       = _rewardsManager.rewardsClaimed(address(_pool), epoch);
                updateRewardsClaimedPerEpoch[address(_pool)][epoch] = _rewardsManager.updateRewardsClaimed(address(_pool), epoch);
            }
    ...
    function _updateExchangeRate(
        ...
        // total the rewards earned pre action
        uint256 totalRewardsEarnedPreAction = _rewardsManager.updateRewardsClaimed(address(_pool), _pool.currentBurnEpoch());
        ...
        try _rewardsManager.updateBucketExchangeRatesAndClaim(address(_pool), keccak256("ERC20_NON_SUBSET_HASH"), indexes_) {
            ...
            require(actorAjnaGain <= _rewardsManager.updateRewardsClaimed(address(_pool), _pool.currentBurnEpoch()) - totalRewardsEarnedPreAction,
