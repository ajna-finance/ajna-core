...
abstract contract UnboundedRewardsPoolHandler is UnboundedPositionPoolHandler {
    ...
    function _stake(
        ...
        require(
            _positionManager.ownerOf(tokenId_) == address(_actor),
            "the actor calling `stake()` is not the owner"
        );

        try _rewardsManager.stake(
            tokenId_
        ) {
            ...
            require(
                _positionManager.ownerOf(tokenId_) == address(_rewardsManager),
                "RW5: owner should be rewardsManager"
            );
        ...
        try _rewardsManager.unstake(
            ...
            for (uint256 epoch = preActionLastClaimedEpoch; epoch <= _pool.currentBurnEpoch(); epoch++) {
                ...
                if (epoch != preActionLastClaimedEpoch) {
                    require(
                        _rewardsManager.isEpochClaimed(tokenId_, epoch) == true,
            ...
            require(
                actorAjnaGain <= totalRewardsEarnedPostAction - totalRewardsEarnedPreAction,
                "RW7: actor's total claimed is greater than rewards earned"
            );

            require(
                actorAjnaGain == contractAjnaBalanceBeforeClaim - _ajna.balanceOf(address(_rewardsManager)),
            ...
            require(
                owner == address(0) && pool == address(0) && lastClaimedEpoch == 0,
    ...
    function _emergencyUnstake(
        ...
        try _rewardsManager.emergencyUnstake(
            tokenId_
        ) {
            // actor should receive tokenId, positionManager loses ownership
            tokenIdsByActor[address(_actor)].add(tokenId_);
            ...
            require(
                totalRewardsEarnedPreAction == totalRewardsEarnedPostAction,
            ...
            require(
                contractAjnaBalanceBeforeClaim == _ajna.balanceOf(address(_rewardsManager)),
                "RW8: ajna balance of rewardsManager changed"
            );

            require(
                actorAjnaBalanceBeforeClaim == _ajna.balanceOf(_actor),
    ...
    function _updateExchangeRate(
        ...
        try _rewardsManager.updateBucketExchangeRatesAndClaim(
            address(_pool),
            ...
            require(
                actorAjnaGain <= _rewardsManager.getUpdateRewardsClaimed(address(_pool), _pool.currentBurnEpoch()) - totalRewardsEarnedPreAction,
    ...
    function _claimRewards(
        ...
        try _rewardsManager.claimRewards(
            ...
            require(
                lastClaimedEpoch == _pool.currentBurnEpoch(),
