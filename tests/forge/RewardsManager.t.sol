...
contract RewardsManagerTest is ERC20HelperContract {
    ...
    function testUpdateExchangeRatesAndClaimRewards() external {
        ...
        vm.expectEmit(true, true, true, true);
        emit UpdateExchangeRates(_updater, address(_poolOne), depositIndexes, 4.089968908133149070 * 1e18);
        _rewardsManager.updateBucketExchangeRatesAndClaim(address(_poolOne), depositIndexes);
        assertEq(_ajnaToken.balanceOf(_updater), 4.089968908133149070 * 1e18);
        ...
        assertEq(rewardsEarned, 40.899689081331421207 * 1e18);
    ...
    function testWithdrawAndClaimRewardsNoExchangeRateUpdate() external {
        ...
        _updateExchangeRates({
            ...
            reward:         4.089968908133149070 * 1e18
        });
        ...
        _assertBurn({
            ...
            burned:    81.799378162662881374 * 1e18,
        ...
        _unstakeToken({
            ...
            reward:            86.809555428378605148 * 1e18,
    ...
    function testWithdrawAndClaimRewardsNoReserveTake() external {
        ...
        _updateExchangeRates({
            ...
            pool:           address(_poolOne),
            depositIndexes: depositIndexes,
            reward:         4.206490995172302285 * 1e18
        });
    }
    ...
    function testMultiPeriodRewardsSingleClaim() external {
        ...
        assertEq(_ajnaToken.balanceOf(_updater), 0);
        vm.expectEmit(true, true, true, true);
        emit UpdateExchangeRates(_updater, address(_poolOne), depositIndexes, 20.449844540665688882 * 1e18);
        _rewardsManager.updateBucketExchangeRatesAndClaim(address(_poolOne), depositIndexes);
        ...
        assertEq(_ajnaToken.balanceOf(_updater), 20.449844540665688882 * 1e18);
        vm.expectEmit(true, true, true, true);
        emit UpdateExchangeRates(_updater, address(_poolOne), depositIndexes, 17.238252336072314418 * 1e18);
        _rewardsManager.updateBucketExchangeRatesAndClaim(address(_poolOne), depositIndexes);
        assertEq(_ajnaToken.balanceOf(_updater), 37.688096876738003300 * 1e18);

        // check available rewards
        rewardsEarned = _rewardsManager.calculateRewards(tokenIdOne, _poolOne.currentBurnEpoch());
        assertEq(rewardsEarned, 376.880968767380567488 * 1e18);
        assertLt(rewardsEarned, Maths.wmul(totalTokensBurned, 0.800000000000000000 * 1e18));
        ...
        assertEq(rewardsEarnedNoUpdate, 376.880968767380567488 * 1e18);
        assertLt(rewardsEarned, Maths.wmul(totalTokensBurned, 0.800000000000000000 * 1e18));
        ...
        vm.expectEmit(true, true, true, true);
        emit UpdateExchangeRates(_updater2, address(_poolOne), depositIndexes, 14.019164349973606338 * 1e18);
        _rewardsManager.updateBucketExchangeRatesAndClaim(address(_poolOne), depositIndexes);
        assertEq(_ajnaToken.balanceOf(_updater2), 14.019164349973606338 * 1e18);

        // check available rewards
        rewardsEarned = _rewardsManager.calculateRewards(tokenIdOne, _poolOne.currentBurnEpoch());
        assertEq(rewardsEarned, 517.072612267116269218 * 1e18);
        ...
        assertEq(_ajnaToken.balanceOf(_updater2), 0);
        vm.expectEmit(true, true, true, true);
        emit UpdateExchangeRates(_updater2, address(_poolOne), depositIndexes, 11.615849155266905358 * 1e18);
        _rewardsManager.updateBucketExchangeRatesAndClaim(address(_poolOne), depositIndexes);
        assertEq(_ajnaToken.balanceOf(_updater2), 11.615849155266905358 * 1e18);

        rewardsEarned = _rewardsManager.calculateRewards(tokenIdOne, _poolOne.currentBurnEpoch());
        assertEq(rewardsEarned, 493.039460320049744942 * 1e18);
    ...
    function testMoveStakedLiquidity() external {
        ...
        vm.expectEmit(true, true, true, true);
        emit UpdateExchangeRates(_minterOne, address(_poolOne), secondIndexes, 4.089968908133149070 * 1e18);
        vm.expectEmit(true, true, true, true);
        emit ClaimRewards(_minterOne, address(_poolOne), tokenId, _epochsClaimedArray(1, 0), 44.989657989464570277 * 1e18);
        // check MoveLiquidity emits
        for (uint256 i = 0; i < firstIndexes.length; ++i) {
            vm.expectEmit(true, true, true, true);
        ...
        changePrank(_minterOne);
        assertEq(_ajnaToken.balanceOf(_minterOne), 44.989657989464570277 * 1e18);
        vm.expectEmit(true, true, true, true);
        emit ClaimRewards(_minterOne, address(_poolOne), tokenId, _epochsClaimedArray(1, 1), 41.730459265783249743 * 1e18);
        _rewardsManager.claimRewards(tokenId, currentBurnEpoch);
    }
    ...
    function testEarlyAndLateStakerRewards() external {
        ...
        _unstakeToken({
            ...
            tokenId:           tokenIdTwo,
            claimedArray:      _epochsClaimedArray(1, 0),
            reward:            39.908019526547891787 * 1e18,
            updateRatesReward: 0
        });
        uint256 minterTwoBalance = _ajnaToken.balanceOf(_minterTwo);
        assertEq(minterTwoBalance, 39.908019526547891787 * 1e18);
        _unstakeToken({
            minter:            _minterThree,
            pool:              address(_poolOne),
            tokenId:           tokenIdThree,
            claimedArray:      _epochsClaimedArray(1, 0),
            reward:            33.248129642902710514 * 1e18,
            updateRatesReward: 0
        });
        uint256 minterThreeBalance = _ajnaToken.balanceOf(_minterThree);
        assertEq(minterThreeBalance, 33.248129642902710514 * 1e18);
    ...
    function testClaimRewardsMultipleDepositsSameBucketsMultipleAuctions() external {
        ...
        _stakeToken(address(_poolOne), _minterTwo, tokenIdTwo);
        assertEq(_ajnaToken.balanceOf(_minterTwo), 8.175422393077328665 * 1e18);
        ...
        assertEq(_ajnaToken.balanceOf(_minterTwo), idTwoRewardsAtTwo + 8.175422393077328665 * 1e18);
    ...
    function testClaimRewardsMultipleDepositsDifferentBucketsMultipleAuctions() external {
        ...
        _updateExchangeRates({
            ...
            pool:           address(_poolOne),
            depositIndexes: depositIndexes,
            reward:         4.089968908133202125 * 1e18
        });
        ...
        _updateExchangeRates({
            ...
            pool:           address(_poolOne),
            depositIndexes: depositIndexes,
            reward:         13.717705175494265742 * 1e18
        });
        ...
        _updateExchangeRates({
            ...
            pool:           address(_poolOne),
            depositIndexes: depositIndexes,
            reward:         27.568516982211953592 * 1e18
        });
        ...
        _assertBurn({
            ...
            interest:  6.443638300196908069 * 1e18,
            burned:    81.799378162664356589 * 1e18
        });
        ...
        _assertBurn({
            ...
            burned:    356.153481672547831475 * 1e18,
        ...
        _assertBurn({
            ...
            epoch:     3,
            timestamp: block.timestamp - 24 hours,
            burned:    907.523821316786357044 * 1e18,
        ...
        _unstakeToken({
            ...
            reward:            75.626985109732100713 * 1e18,
            updateRatesReward: 0
        });
        ...
        _unstakeToken({
            ...
            tokenId:           tokenIdTwo,
            claimedArray:      _epochsClaimedArray(3, 0),
            reward:            378.134925548660503562 * 1e18,
            updateRatesReward: 0
        });
    }
    ...
    function testWithdrawAndClaimRewards() external {
        ...
        assertEq(managerBalance, 4.931444746461898520 * 1e18);

        changePrank(_minterOne);
        vm.expectEmit(true, true, true, true);
        emit ClaimRewards(_minterOne, address(_poolOne), tokenIdOne, _epochsClaimedArray(1, 0), 40.899689081331421207 * 1e18);
        vm.expectEmit(true, true, true, true);
        emit Unstake(_minterOne, address(_poolOne), tokenIdOne);
        _rewardsManager.unstake(tokenIdOne);
        ...
        _rewardsManager.unstake(tokenIdOne);
        assertEq(_positionManager.ownerOf(tokenIdOne), _minterOne);
        assertEq(_ajnaToken.balanceOf(_minterOne), 40.899689081331421207 * 1e18);
    ...
    function testMultiplePools() external {
        ...
        assertEq(_ajnaToken.balanceOf(_minterOne), 0);
        vm.expectEmit(true, true, true, true);
        emit UpdateExchangeRates(_minterOne, address(_poolOne), depositIndexesOne, 4.089968908133149070 * 1e18);
        ...
        assertEq(_ajnaToken.balanceOf(_minterOne), 4.089968908133149070 * 1e18);
