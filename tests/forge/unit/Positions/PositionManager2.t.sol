...
contract PositionManagerERC20PoolTest is PositionManagerERC20PoolHelperContract {
    ...
    function testIsAjnaPool() external {
        assertTrue(_positionManager.isAjnaPool(address(_pool), keccak256("ERC20_NON_SUBSET_HASH")));
    }

    function testIsAjnaPoolWithoutSubsetHash() external {
        assertTrue(_positionManager.isAjnaPool(address(_pool)));
    }

    function testIsAjnaPoolAddedWithoutSubsetHash() external {
        _positionManager.addSubsetPool(address(_pool), keccak256("ERC20_NON_SUBSET_HASH"));
        assertTrue(_positionManager.isAjnaPool(address(_pool)));
    }
