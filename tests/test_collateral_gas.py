...
def test_add_remove_collateral_gas(
    ...
    with test_utils.GasWatcher(["addQuoteToken", "addCollateral", "removeCollateral"]):
        ...
        tx_add_collateral = mkr_dai_pool.addCollateral(
            100 * 10**27, {"from": borrowers[0]}
        )
        mkr_dai_pool.borrow(20_000 * 10**45, 2500 * 10**18, {"from": borrowers[0]})
        tx_remove_collateral = mkr_dai_pool.removeCollateral(
            10 * 10**27, {"from": borrowers[0]}
        )
...
def test_claim_collateral_gas(
    ...
    with test_utils.GasWatcher(
        ...
        mkr_dai_pool.addQuoteToken(
            lender, 3_000 * 10**45, bucket_math.indexToPrice(1663), {"from": lender}
        )
        mkr_dai_pool.addQuoteToken(
            lender, 4_000 * 10**45, bucket_math.indexToPrice(1606), {"from": lender}
        )
        mkr_dai_pool.addQuoteToken(
            lender, 5_000 * 10**45, bucket_math.indexToPrice(1386), {"from": lender}
        )

        mkr_dai_pool.addCollateral(100 * 10**27, {"from": borrower})
        mkr_dai_pool.borrow(4_000 * 10**45, 3000 * 10**18, {"from": borrower})

        # bidder purchases some of the middle bucket
        mkr_dai_pool.purchaseBid(
            1_500 * 10**45, bucket_math.indexToPrice(1606), {"from": bidder}
        )

        tx = mkr_dai_pool.claimCollateral(
            lender, 0.4 * 10**27, bucket_math.indexToPrice(1606), {"from": lender}
        )
