...
def test_add_remove_collateral_gas(
    ...
    with test_utils.GasWatcher(["addQuoteToken", "addCollateral", "removeCollateral"]):
        mkr_dai_pool.addQuoteToken(20_000 * 1e18, 1708, {"from": lenders[0]})
...
def test_claim_collateral_gas(
    ...
    with test_utils.GasWatcher(
        ...
        mkr_dai_pool.addQuoteToken(4_000 * 1e18, 1606, {"from": lender})
        mkr_dai_pool.addQuoteToken(5_000 * 1e18, 1386, {"from": lender})
        ...
        mkr_dai_pool.purchaseBid(1_500 * 1e18, 1606, {"from": bidder})

        tx = mkr_dai_pool.claimCollateral(0.4 * 1e18, 1606, {"from": lender})
