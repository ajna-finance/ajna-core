...
def test_quote_deposit_gas_below_hdp(
    ...
    with test_utils.GasWatcher(["addQuoteToken"]):
        ...
        for i in reversed(range(1000, 1020)):
            tx = mkr_dai_pool.addQuoteToken(100 * 1e18, i, {"from": lenders[0]})
            txes.append(tx)
...
def test_quote_removal_from_lup_with_reallocation(
    ...
    with test_utils.GasWatcher(
        ...
        mkr_dai_pool.addQuoteToken(3_400 * 1e18, 1663, {"from": lender})
        mkr_dai_pool.addQuoteToken(3_400 * 1e18, 1606, {"from": lender})
        ...
        tx = mkr_dai_pool.removeQuoteToken(1_000 * 1e18, 1663, {"from": lender})
...
def test_quote_removal_below_lup(
    ...
    with test_utils.GasWatcher(
        ...
        mkr_dai_pool.addQuoteToken(5_000 * 1e18, 1663, {"from": lender})
        mkr_dai_pool.addQuoteToken(5_000 * 1e18, 1606, {"from": lender})
        mkr_dai_pool.addQuoteToken(5_000 * 1e18, 1524, {"from": lender})
        ...
        # lender removes 1000 DAI
        tx = mkr_dai_pool.removeQuoteToken(1_000 * 1e18, 1606, {"from": lender})
