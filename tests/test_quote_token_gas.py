...
def test_quote_removal_from_lup_with_reallocation(
    ...
    with test_utils.GasWatcher(
        ...
        mkr_dai_pool.addQuoteToken(
            lender, 3_400 * 10**45, bucket_math.indexToPrice(1663), {"from": lender}
        )
        mkr_dai_pool.addQuoteToken(
            lender, 3_400 * 10**45, bucket_math.indexToPrice(1606), {"from": lender}
        )

        # borrower takes a loan of 3000 DAI
        mkr_dai_pool.addCollateral(100 * 10**27, {"from": borrower})
        mkr_dai_pool.borrow(3_000 * 10**45, 4000 * 10**18, {"from": borrower})

        # lender removes 1000 DAI
        tx = mkr_dai_pool.removeQuoteToken(
            lender, 1_000 * 10**45, bucket_math.indexToPrice(1663), {"from": lender}
        )
...
def test_quote_removal_below_lup(
    ...
    with test_utils.GasWatcher(
        ...
        mkr_dai_pool.addQuoteToken(
            lender, 5_000 * 10**45, bucket_math.indexToPrice(1663), {"from": lender}
        )
        mkr_dai_pool.addQuoteToken(
            lender, 5_000 * 10**45, bucket_math.indexToPrice(1606), {"from": lender}
        )
        mkr_dai_pool.addQuoteToken(
            lender, 5_000 * 10**45, bucket_math.indexToPrice(1524), {"from": lender}
        )
        ...
        # lender removes 1000 DAI
        tx = mkr_dai_pool.removeQuoteToken(
            lender, 1_000 * 10**45, bucket_math.indexToPrice(1606), {"from": lender}
        )
