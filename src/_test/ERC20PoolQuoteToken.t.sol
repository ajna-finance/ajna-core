...
contract ERC20PoolQuoteTokenTest is DSTestPlus {
    ...
    function testDepositQuoteToken() public {
        ...
        assertEq(pool.hdp(), -3232);

        // test 10000 DAI deposit at price of 1 MKR = 4000.927678580567537368 DAI, bucket index 1663
        ...
        assertEq(downPrice, 99836282890);
        assertEq(deposit, 10_000 * 1e18);
        assertEq(debt, 0);
        assertEq(snapshot, 1 * 1e18);
        assertEq(lpOutstanding, 10_000 * 1e18);
        assertEq(pool.lpBalance(address(lender), 1663), 10_000 * 1e18);

        // test 20000 DAI deposit at price of 1 MKR = 2000.221618840727700609 DAI, bucket index 1524
        vm.expectEmit(true, true, false, true);
        ...
        lender.addQuoteToken(pool, 20_000 * 1e18, 1524);
        // check pool hdp and balances
        assertEq(pool.hdp(), 1663);
        ...
        ) = pool.bucketAt(2_000.221618840727700609 * 1e18);
        assertEq(price, 2_000.221618840727700609 * 1e18);
        assertEq(upPrice, 4_000.927678580567537368 * 1e18);
        assertEq(downPrice, 99836282890);
        ...
        assertEq(pool.lpBalance(address(lender), 1524), 20_000 * 1e18);
        ...
        // test 30000 DAI deposit at price of 1 MKR = 3010.892022197881557845 DAI, bucket index 1606
        vm.expectEmit(true, true, false, true);
        ...
        lender.addQuoteToken(pool, 30_000 * 1e18, 1606);
        // check pool hdp and balances
        assertEq(pool.hdp(), 1663);
        ...
        assertEq(price, 3_010.892022197881557845 * 1e18);
        assertEq(upPrice, 4_000.927678580567537368 * 1e18);
        assertEq(downPrice, 2_000.221618840727700609 * 1e18);
        ...
        assertEq(pool.lpBalance(address(lender), 1606), 30_000 * 1e18);
        // check hdp down price pointer updated
        (, upPrice, downPrice, , , , , ) = pool.bucketAt(
            4_000.927678580567537368 * 1e18
        );
        assertEq(upPrice, 4_000.927678580567537368 * 1e18);
        assertEq(downPrice, 3_010.892022197881557845 * 1e18);
        // check 2_000.221618840727700609 down price pointer updated
        (, upPrice, downPrice, , , , , ) = pool.bucketAt(
            2_000.221618840727700609 * 1e18
        );
        assertEq(upPrice, 3_010.892022197881557845 * 1e18);
        assertEq(downPrice, 99836282890);

        // test 40000 DAI deposit at price of 1 MKR = 5007.644384905151472283 DAI, bucket index 1708
        // hdp should be updated to 5007.644384905151472283 DAI
        // hdp next price should be 4_000.927678580567537368 DAI
        vm.expectEmit(true, true, false, true);
        ...
        assertEq(pool.lpBalance(address(lender), 1708), 40_000 * 1e18);
    }

    function testRemoveQuoteTokenNoLoan() public {
        // lender deposit 10000 DAI at price 4000.927678580567537368
        lender.addQuoteToken(pool, 10_000 * 1e18, 1663);

        // should revert if trying to remove more than lended
        vm.expectRevert("ajna/amount-greater-than-claimable");
        lender.removeQuoteToken(pool, 20_000 * 1e18, 1663);
        ...
        assertEq(pool.lpBalance(address(lender), 1663), 0 * 1e18);
    }
    ...
    function testRemoveQuoteTokenUnpaidLoan() public {
        ...
        emit RemoveQuoteToken(
            address(lender),
            4_000.927678580567537368 * 1e18 * 1e18,
            4_000 * 1e18,
            1663
        );
    ...
    function testRemoveQuoteTokenWithDebtReallocation() public {
        ...
        // (1663, 4_000.927678580567537368) and (1606, 3_010.892022197881557845)
        lender.addQuoteToken(pool, 3_400 * 1e18, 1663);
        lender.addQuoteToken(pool, 3_400 * 1e18, 1606);
        ...
        lender.removeQuoteToken(pool, 1_000 * 1e18, 1663);
    ...
    function testRemoveQuoteTokenBelowLup() public {
        ...
        lender.addQuoteToken(pool, 5_000 * 1e18, 1606);
        lender.addQuoteToken(pool, 5_000 * 1e18, 1524);
        ...
        // lender removes 1000 DAI under the lup - from bucket 3_010.892022197881557845
        vm.expectEmit(true, true, false, true);
        emit Transfer(address(pool), address(lender), 1_000 * 1e18);
        emit RemoveQuoteToken(
            address(lender),
        ...
        lender.removeQuoteToken(pool, 1_000 * 1e18, 1606);
    ...
    function testRemoveQuoteUndercollateralizedPool() public {
        ...
        // (1386, 1_004.989662429170775094) and (924, 100.332368143282009890)
        lender.addQuoteToken(pool, 5_000 * 1e18, 1386);
        lender.addQuoteToken(pool, 5_000 * 1e18, 924);
        ...
        lender.removeQuoteToken(pool, 2_000 * 1e18, 1386);
    }
}
