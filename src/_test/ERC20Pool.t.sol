...
contract ERC20PoolTest is DSTestPlus {
   ...
   function testManipulationMitigations() external {
        ...
        assertEq(_pool.getPoolAverageDebtAmount(), 0);
        assertEq(_pool.totalLoans(),               0);
        ...
        assertEq(_pool.getPoolAverageDebtAmount(), 100.000961538461538462 * 1e18);
        assertEq(_pool.totalLoans(),               1);

        // should fail if trying to borrow amount < 10% of pool average debt amount
        vm.expectRevert("P:B:AMT_LT_AVG_DEBT");
        _borrower.borrow(_pool, 10 * 1e18, 3000 * 1e18);

        // loans accumulator should be incremented only if new borrower
        _borrower.borrow(_pool, 100 * 1e18, 3000 * 1e18);
        assertEq(_pool.getPoolAverageDebtAmount(), 200.001923076923076924 * 1e18);
        assertEq(_pool.totalLoans(),               1);

        _borrower1.borrow(_pool, 100 * 1e18, 3000 * 1e18);
        assertEq(_pool.getPoolAverageDebtAmount(), 150.001442307692307693 * 1e18);
        assertEq(_pool.totalLoans(),               2);

        _borrower2.borrow(_pool, 200 * 1e18, 3000 * 1e18);
        assertEq(_pool.getPoolAverageDebtAmount(), 166.667948717948717949 * 1e18);
        assertEq(_pool.totalLoans(),               3);
        ...
        vm.expectRevert("P:R:AMT_LT_AVG_DEBT");
         _borrower2.repay(_pool, 190 * 1e18);

        _borrower2.repay(_pool, 100 * 1e18);
        assertEq(_pool.getPoolAverageDebtAmount(), 133.334615384615384616 * 1e18);
        assertEq(_pool.totalLoans(),               3);
        ...
        _lender2.addQuoteToken(_pool, address(_lender), 10 * 1e18, _p2850);
        ...
        assertEq(_pool.getPoolAverageDebtAmount(), 100.001442307692307693 * 1e18);
        assertEq(_pool.totalLoans(),               2);
        ...
        assertEq(_pool.getPoolAverageDebtAmount(), 80.000961538461538462 * 1e18);
        assertEq(_pool.totalLoans(),               1);
