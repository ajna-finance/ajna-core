...
contract ERC721ScaledBorrowTest is ERC721HelperContract {
    ...
    function testSubsetPurchaseQuote() external {
        ...
        // check bucket state
        ( , uint256 quote, uint256 collateral, uint256 lpb, , ) = _poolUtils.bucketInfo(address(_subsetPool), 2550);
        assertEq(quote,      0);
        ...
        // check bucket state
        (, quote, collateral, lpb, , ) = _poolUtils.bucketInfo(address(_subsetPool), testIndex);
    ...
    function testSubsetPurchaseQuoteWithDebt() external {
        ...
        // check bucket state
        ( , uint256 quote, uint256 collateral, uint256 lpb, , ) = _poolUtils.bucketInfo(address(_subsetPool), 2350);
        ...
        // check bucket state
        ( , quote, collateral, lpb, , ) = _poolUtils.bucketInfo(address(_subsetPool), 2350);
        assertEq(quote,      0);
