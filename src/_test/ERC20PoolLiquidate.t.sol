...
contract ERC20PoolLiquidateTest is DSTestPlus {
    ...
    function testLiquidate() external {
        ...
        assertEq(_pool.getPoolCollateralization(), 165.648478682707543147894545392 * 1e27);
        assertEq(
        ...
        assertEq(_pool.getBorrowerCollateralization(collateralDeposited, borrowerDebt), collateralization);
        assertEq(_pool.getPoolCollateralization(), 1.688928197078580499815 * 1e27);
        assertEq(
            _pool.getBorrowerCollateralization(collateralDeposited, borrowerDebt),
            collateralization
        );
        ...
        emit Liquidate(
            address(_borrower),
            11_000.143012091382543917 * 1e18,
            1.209062604930973351 * 1e18
        );
        _lender.liquidate(_pool, address(_borrower));
        ...
        assertEq(_pool.lup(),                        priceLow);
        assertEq(_quote.balanceOf(address(_pool)),   9_000 * 1e18);
        assertEq(_pool.getPoolCollateralization(),   20.145568336024044703561024660 * 1e27);
    ...
    function testLiquidateScenario1NoTimeWarp() external {
        ...
        assertEq(_pool.getEncumberedCollateral(borrowerDebt), collateralEncumbered);
        assertEq(_pool.getBorrowerCollateralization(collateralDeposited, borrowerDebt), collateralization);
        assertEq(_pool.getPoolCollateralization(), 1.559010643457151230598461538 * 1e27);
        assertEq(
            _pool.getBorrowerCollateralization(collateralDeposited, borrowerDebt),
            collateralization
        );
        ...
        assertEq(_pool.getPoolCollateralization(), 20.112155247098450521242599672 * 1e27);
        assertEq(_pool.getPoolActualUtilization(), 1 * 1e18);
    }
    ...
    function testLiquidateScenario1TimeWarp() external {
        ...
        assertEq(collateralization,    0.014270292476495861630562326 * 1e27);
        assertEq(borrowerInflator,     1 * 1e27);
        ...
        assertEq(_pool.getPoolCollateralization(), 1.345606594670868805846262079 * 1e27);
        ...
        assertEq(_pool.getPoolCollateralization(), 20.085527706983651821079383351 * 1e27);
