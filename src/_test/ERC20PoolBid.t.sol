...
contract ERC20PoolBidTest is DSTestPlus {
    ...
    function testPurchaseBidPartialAmount() public {
        ...
        // (1663, 4_000.927678580567537368), (1606, 3_010.892022197881557845) and (1386, 1_004.989662429170775094)
        lender.addQuoteToken(pool, 3_000 * 1e18, 1663);
        lender.addQuoteToken(pool, 3_000 * 1e18, 1606);
        lender.addQuoteToken(pool, 3_000 * 1e18, 1386);
    ...
    function testPurchaseBidEntireAmount() public {
        ...
        lender.addQuoteToken(pool, 1_000 * 1e18, 1663);
        lender.addQuoteToken(pool, 1_000 * 1e18, 1606);
        lender.addQuoteToken(pool, 5_000 * 1e18, 1524);

        // borrower takes a loan of 1000 DAI from bucket 4_000.927678580567537368
        borrower.addCollateral(pool, 100 * 1e18);
        ...
        // purchase 1000 bid - entire amount in 4_000.927678580567537368 bucket
    ...
    function testPurchaseBidCannotReallocate() public {
        ...
        lender.addQuoteToken(pool, 1_000 * 1e18, 1663);
        lender.addQuoteToken(pool, 1_000 * 1e18, 1606);
        lender.addQuoteToken(pool, 500 * 1e18, 1524);
    ...
    function testPurchaseBidUndercollateralized() public {
        // lender deposit DAI in 3 buckets
        // (1663, 4_000.927678580567537368), (1606, 3_010.892022197881557845) and (0, 1)
        lender.addQuoteToken(pool, 1_000 * 1e18, 1663);
        lender.addQuoteToken(pool, 1_000 * 1e18, 1606);
        lender.addQuoteToken(pool, 2_000 * 1e18, 0);
