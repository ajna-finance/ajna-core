...
contract ERC20Pool is IPool, BorrowerManager, Clone, LenderManager {
    ...
    function borrow(uint256 amount_, uint256 limitPrice_) external override {
        ...
        if (borrower.debt != 0) {
            borrower.debt += getPendingInterest(borrower.debt, inflatorSnapshot, borrower.inflatorSnapshot);
        }
    ...
    function repay(uint256 maxAmount_) external override {
        ...
        accumulatePoolInterest();
        uint256 borrowerDebt = borrower.debt + getPendingInterest(borrower.debt, inflatorSnapshot, borrower.inflatorSnapshot);
