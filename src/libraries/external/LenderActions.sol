...
library LenderActions {
    ...
    function _removeMaxCollateral(
        ...
        if (requiredLPs <= lenderLpBalance) {
            ...
            collateralAmount_ = Maths.wmul(Maths.rrdivw(lenderLpBalance, requiredLPs), bucketCollateral);
        }
    ...
    function _removeMaxDeposit(
        ...
        // scaledRemovedAmount is constrained by the de-scaled maxAmount(in QT), the scaledDeposit constraint, and
        // the lender LPB exchange rate in scaled deposit-to-LPB for the bucket:
        // scaledRemovedAmount = min ( maxAmount_, scaledDeposit, lenderLPsBalance*scaledExchangeRate)
        // redeemedLPs_ = min ( maxAmount_/scaledExchangeRate, scaledDeposit/scaledExchangeRate, lenderLPsBalance)

        uint256 scaledLpConstraint = Maths.rmul(params_.lpConstraint, exchangeRate);
        if (
            params_.depositConstraint < scaledDepositAvailable &&
            Maths.wadToRay(params_.depositConstraint) < scaledLpConstraint
        ) {
            // depositConstraint is binding constraint
            removedAmount_ = params_.depositConstraint;
            redeemedLPs_   = Maths.wrdivr(removedAmount_, exchangeRate);
        } else if (Maths.wadToRay(scaledDepositAvailable) < scaledLpConstraint) {
            ...
            removedAmount_ = Maths.rayToWad(Maths.rmul(redeemedLPs_, exchangeRate));
        }
